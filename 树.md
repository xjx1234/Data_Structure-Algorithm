...
- 删除非叶子节点，直接用后继节点覆盖当前节点，再删除用来覆盖的后续节点；

  

  ![](https://my-data-structure.oss-cn-shanghai.aliyuncs.com/tree/tree-19.png)

- 删除的节点为叶子节点

  - 当前节点不是2节点，则直接删除需要删除的值的节点；

  - 当前节点为2节点，则需要分情况：

    1.  当前节点的双亲节点是2-节点、兄弟节点是3-节点，将双亲节点移动到当前位置，再将兄弟节点中最接近当前位置的key移动到双亲节点中；

       ![](https://my-data-structure.oss-cn-shanghai.aliyuncs.com/tree/tree-20.png)

       如上图，父节点4为两节点，兄弟节点6-7为三节点，则需要将双亲节点4移到待删除节点1出，将兄弟节点6-7节点最接近删除节点1值的6节点移到双亲节点4处

    2. 当前节点的双亲节点是2-节点、兄弟节点也是2-节点，先通过移动兄弟节点的中序遍历直接后驱到兄弟节点，以使兄弟节点变为3-节点；再进行1)的操作；
